...
class ComplainsController extends Controller {
    ...
    public function actionAdd() {
        if (Yii::app()->request->isPostRequest) {
            $model = new ComplainsMaster();
            // Uncomment the following line if AJAX validation is needed
            $this->performAjaxValidation($model, "form-complains");

            if (isset($_POST['ComplainsMaster'])) {
                $model->attributes = $_POST['ComplainsMaster'];
                if ($model->validate()) {
                ...
                $this->redirect(array("/admin/complains"));
            }

            $outputJs = Yii::app()->request->isAjaxRequest;
            $this->renderPartial('_form_complains', array('model' => $model), false, $outputJs);
        } else
    ...
    public function actionUpdate($id) {
        if (Yii::app()->request->isPostRequest) {
            $model = $this->loadModel($id, "ComplainsMaster");
            // Uncomment the following line if AJAX validation is needed
            $this->performAjaxValidation($model, "form-complains");
